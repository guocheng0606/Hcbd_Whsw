...
import com.android.hcbd.whsw.viewholder.ServiceParamViewHolder;
...
public class ServiceParamActivity extends BaseActivity implements RecyclerArrayAdapter.OnLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_service_param);
        ButterKnife.bind(this);
        EventBus.getDefault().register(this);

        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setTitle(R.string.server_parameters);

        initView();
        if (TextUtils.isEmpty(MyApplication.getInstance().getPowerStr("服务器参数"))) {
            swipeRefreshLayout.setVisibility(View.GONE);
            layoutPower.setVisibility(View.VISIBLE);
            tvPower.setText("您没有访问服务器参数权限,请与管理员联系！");
        }else{
            initHttpData();
        }
        initListener();
    ...
    private void initListener() {
        adapter.setOnItemClickListener(new RecyclerArrayAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position) {
                Intent intent = new Intent(ServiceParamActivity.this, ServiceParamEditActivity.class);
    ...
    private void initView() {
        ...
        recyclerView.setAdapterWithProgress(adapter = new RecyclerArrayAdapter<ConfigureInfo>(this) {
            ...
            public BaseViewHolder OnCreateViewHolder(ViewGroup parent, int viewType) {
                return new ServiceParamViewHolder(parent);
            }
        });
